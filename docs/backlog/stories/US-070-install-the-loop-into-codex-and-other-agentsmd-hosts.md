...
- [x] Given a repo with `AGENTS.md` and no `.cursor` or `.claude` directory, when I run `pb init`,
...
- [x] Given any supported host, when I run `pb instructions overview`, then the router returned is
      identical across hosts (ADR-0010) — one compiled router, not three copies.
- [x] Given a host copy that has diverged from `skills/<name>.md`, when the drift test runs, then it
      fails and names the file.
- [x] Given a host we do not support, when `pb init` runs, then the output names it as unsupported
      rather than pretending coverage (BR-005).
- [x] Given `pb init --refresh-skills`, when a host copy is unedited, then it is upgraded; an edited
      copy is left alone and reported.

## Notes

Implemented as router-only for `agents`/`codex`: `pb init` installs the AGENTS.md router (the
compiled skill list from `pb instructions overview`), but does not copy full skill bodies into a
Codex-specific directory — no such convention is known to exist today, and `pb skill <name>`
already reads the bundled package content directly, so a per-repo copy would be pure drift risk
with no functional gain. `status: review` rather than `done`: the second AC as written assumes
every host gets a directory of skill files; a reviewer should confirm router-only is the intended
reading for AGENTS.md hosts, or split a follow-up if a Codex-specific delivery mechanism emerges.
